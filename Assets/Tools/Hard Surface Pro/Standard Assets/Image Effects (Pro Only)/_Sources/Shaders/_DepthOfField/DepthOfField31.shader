// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'
 ...
 Shader "Hidden/DepthOfField31" {
	...
	half4 fragDofApplySimple(v2fDofApply i) : COLOR 
	{
		half4 finalColor = half4(0.0,0.0,0.0,1.0);
		half4 tapHigh = tex2D(_MainTex, i.uv.xy); 		
		half4 tapLow = tex2D(_BgLowRez, i.uv.xy);
	...
	half4 fragDofApplyVerySimple(v2fDofApply i) : COLOR 
	{
		half4 finalColor = half4(0.0,0.0,0.0,1.0);
		half4 tapHigh = tex2D(_MainTex, i.uv.xy); 		
		half4 tapLow = tex2D(_BgLowRez, i.uv.xy);
	...
	half4 fragPreBg(v2f i) : COLOR 
	{
		float d = tex2D (_CameraDepthTexture, i.uv1.xy);
		d = Linear01Depth(d);
		half preDof = 0.0; 
		
		focalDistance01 = (focalDistance01+focalSize);
		
		if(d>focalDistance01) 
			preDof = saturate(d-focalDistance01) * _CurveParams.y/focalDistance01;
	...
	half4 fragCocBgCenter(v2f i) : COLOR 
	{
		return half4(1,0,1,0);
	}
	
	half4 fragCocFgCenter(v2f i) : COLOR 
	{
		return half4(1,0,1,0);		
	}
	...
	{	
		...
		if(d>focalDistance01)
			...
			preDof = saturate(focalDistance01-d)  * _CurveParams.x/focalDistance01;
		
		color.a = saturate(preDof * focalFalloff);	
		return color;
	}		
	
	half4 fragPreFgBgCenter(v2f i) : COLOR 
	{
		return half4(1,0,1,0);

	}

	half4 fragPreFgBg(v2f i) : COLOR 
	{ 
		float preDof = 0.0;
	...
	half4 fragFgBlurBokeh (v2fFgBlur i) : COLOR 
	{					
	...
	half4 fragFgBlurSimple (v2fFgBlur i) : COLOR 
	{					
		half4 blurredColor = float4 (0,0,0,0);
